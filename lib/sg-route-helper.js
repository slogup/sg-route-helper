...
var corePackagePath = path.resolve(rootPath, 'package.json');
var appPackagePath = path.resolve(rootPath, './app/package.json');
var appVersion = "";
if (fs.existsSync(appPackagePath)) {
    appVersion = require(appPackagePath).version;
}
...
module.exports.prepareParam = function (contentName) {
    return function (req, res, next) {
        req.preparedParam = {
            params: {
                version: {
                    core: require(corePackagePath).version
                },
                session: (req.user && req.user.toSecuredJSON && req.user.toSecuredJSON()) || {},
        ...
        if (appVersion) {
            req.preparedParam.params.version.app = appVersion;
        }

        req.preparedParam.params.url = url.parse(req.protocol + '://' + req.get('host') + req.originalUrl);
        req.preparedParam.params.url.query = req.query;
        next();
    };
};